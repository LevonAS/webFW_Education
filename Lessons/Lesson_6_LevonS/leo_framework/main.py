...
from .proc_requests import GetRequests, PostRequests
...
class Framework:
    ...
    def __call__(self, environ, start_response):
        ...
        if method == 'POST':
            ...
            if path == "/contact/" and not re.findall(number_re, data['your_email']):
    ...
    def decode_value(data):
        """ Метод меняет не транслируемые символы в запросе на правильные """
        new_data = {}
        for k, v in data.items():
            # print("///m.decode_value/ v: ", v)
            val = bytes(v.replace('%', '=').replace("+", " "), 'UTF-8')
            val_decode_str = decodestring(val).decode('UTF-8')
            # print("///m.decode_value/ v_decode_str: ", val_decode_str)
...
# http://192.168.42.140:8000/contact/?id=1&category=10
# Пример полученного GET-запроса с параметрами
